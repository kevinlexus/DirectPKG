...
begin
  ...
  select t.aoguid from fias_addr t
   ...
   --join t_org_tp tp on o.fk_orgtp=tp.id and tp.cd='√ород' -убрал, так как может быть много городов в данном – ÷
...
begin
  --загрузить только новые дома, которые соответствуют отобранным пользовател€м улицам
