create or replace package scott.C_GET_PAY is
  --глобальная переменная определяющая текущий номер компьютера пользователя
  TYPE rep_refcursor IS REF CURSOR;
  type ccur is ref cursor;

  --флаг распределения оплаты
  g_flag_upd number;
...
create or replace package body scott.C_GET_PAY is
  ...
  begin
    ...
    if nvl(iscorrect_, 0) = 1 then
      ...
      values
        (lpad(lsk_, 8, '0'), summa_, penya_, oper_, dopl_, nink_, dat_ink_, init.get_nkom(), dtplat_, nkvit_, sysdate, id_, iscorrect_, num_doc_, dat_doc_);
      ...
      values
        (lpad(lsk_, 8, '0'), summa_, oper_, dopl_, nink_, dat_ink_, init.get_nkom(), dtplat_, nkvit_, sysdate, id_, iscorrect_, num_doc_, dat_doc_);
    else
      -- обычная оплата (0)
      insert into c_kwtp
        (lsk, summa, penya, oper, dopl, nink, dat_ink, nkom, dtek, nkvit, ts, id, iscorrect, num_doc, dat_doc)
      values
        (lpad(lsk_, 8, '0'), summa_, penya_, oper_, (select period
             from params), nink_, dat_ink_, init.get_nkom(), dtplat_, nkvit_, sysdate, id_, iscorrect_, num_doc_, dat_doc_);
    end if;
