create or replace package body scott.C_DIST_PAY2 is
...
l_mg params.period%type;
begin
--начальное формирование таблицы задолжности текущего месяца
--первый день месяца
l_mg:=p_mg;
...
begin
      ...
      select rec_summ(b.usl, b.org, b.summa, 0) bulk collect --убрал редирект - тормозит у полыс
          ...
          select a.usl, a.org, sum(a.summa) as summa from   
            ...
            select t.usl, t.org, t.summa from  
                v_changes_for_dist_pay t -- добавить перерасчёты уже здесь, чтобы деньги распр. с учётом текущ перерасч!
  ...
  --по текущему начислению (по тек начисл, без коррект проводки)
    select rec_summ(n.usl, n.org, sum(t.summa), 0) bulk collect --убрал редирект - тормозит у полыс
          into t_summ
        from c_charge t, nabor n
        where t.lsk=p_rec.lsk
        and t.lsk=n.lsk
    ...
    select rec_summ(d.usl, d.org, sum(d.summa), 0) bulk collect
          into t_summ from
            (select t.usl, t.org, nvl(t.indebet,0)+nvl(t.inkredit,0) as summa
              ...
              and t.mg=p.period
            union all
            select n.usl, n.org as org, t.summa --убрал редирект - тормозит у полыс
     ...
     p_distr not in (15,16,17) then
      ...
      insert into temp_prep  --поставил 20.04.15 в 15:08
       ...
       with a as (select t.usl, t.org, sum(t.summa) as summa from temp_prep t
            ...
            (select t.usl, t.org, nvl(t.indebet,0)+nvl(t.inkredit,0) as summa
              ...
              and t.mg=p.period
            union all
            select n.usl, n.org as org, t.summa
              from c_charge t, nabor n  --прибавить текущее начисление
              where t.lsk=p_rec.lsk
              and t.lsk=n.lsk
              and t.usl=n.usl
      ...
      insert into temp_prep  --поставил 20.04.15 в 15:08
       ...
       with a as (select t.usl, t.org, sum(t.summa) as summa from temp_prep t
        ...
        from v_changes_for_saldo t where t.lsk=p_rec.lsk and t.org is not null --добавить перерасчеты
            union all
            select n.usl, n.org as org, t.summa
              from c_charge t, nabor n  --прибавить текущее начисление
              where t.lsk=p_rec.lsk
