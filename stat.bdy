...
--    l_rec l_cur%ROWTYPE;
   ...
   --детализация до квартир
    ...
    t.trest, s.reu, ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name) as name,
    ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name)||', '||NVL(LTRIM(s.nd,'0'),'0')||'-'||NVL(LTRIM(s.kw,'0'),'0') AS predpr_det,
    det.ord1,
    k1.fio,
    ...
    and exists
       (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
        ...
        and i.sel=1)
    and ((var_=3 and
           s.reu = reu_ and case when s.for_reu is null and mg_ < '201907' then 1 -- до 201907 поле for_reu - не заполнялось ред.12.11.20 - Кис. жаловались, что выходят лишние дома
                                             when p_for_reu is null then 1
                                             when s.for_reu = p_for_reu then 1
                                             else 0 end =1  
           and s.kul = kul_
           and s.nd = nd_)
          or (var_=2 and s.reu=reu_ and case when s.for_reu is null and mg_ < '201907' then 1 -- до 201907 поле for_reu - не заполнялось ред.12.11.20 - Кис. жаловались, что выходят лишние дома
                                             when p_for_reu is null then 1
                                             when s.for_reu = p_for_reu then 1
                                             else 0 end =1  
           )                        
...
/*    insert into temp_stat2
      ...
      select --для отображения объемов по ОДПУ
                        ...
                        case when d.dist_tp<>4 and nvl(d.kub,0) = 0 then 'есть, нет объема'
                        when d.dist_tp<>4 and nvl(d.kub,0) <> 0 then 'есть'
                        else 'нет' end as odpu_ex, nvl(m.ishotpipeinsulated,0) as ishotpipeinsulated, 
                        nvl(m.istowelheatexist,0) as istowelheatexist, 
                        case when d.usl not in ('053','054') then d.kub - (d.kub_norm + d.kub_sch + d.kub_ar)
                          else 0 end as kr_soi, -- кр на сои (только для первой строки) и не для 053 и 054 услуг
                        d.kub_norm + d.kub_sch + d.kub_ar as fact_cons  -- факт.потребление (только для первой строки)
                        from a_vvod d 
                        join arch_kart h on h.house_id=d.house_id and h.mg=d.mg and d.mg between mg_ and mg1_ and h.psch not in (8,9)
                        join a_nabor2 n on d.id=n.fk_vvod and d.usl=n.usl and h.lsk=n.lsk and d.mg between n.mgFrom and n.mgTo
                        join s_reu_trest s on h.reu=s.reu 
                        join usl u on d.usl=u.usl
                        left join a_vvod m on d.house_id=m.house_id and d.mg=m.mg and m.usl='162' -- ред.30.08.21 по этой услуге, для других услуг определять наличие полотенцесуш.
                  where case when var_=3 and h.reu = reu_
                           and h.kul = kul_
                           and h.nd = nd_ then 1
                           when var_=2 and h.reu=reu_ then 1
                           when var_=1 and s.trest=trest_ then 1
   ...
   --детализация до домов
        ...
        s.kul,
    t.trest, s.reu, ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name) as name,
    ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name)||', '||NVL(LTRIM(s.nd,'0'),'0') AS predpr_det,
   ...
   (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
    ...
    and exists
       (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
        ...
        and ((var_=3 and
           ...
           and s.kul = kul_
           and s.nd = nd_) or
         (var_=2 and s.reu=reu_ and case when s.for_reu is null and mg_ < '201907' then 1 -- до 201907 поле for_reu - не заполнялось ред.12.11.20 - Кис. жаловались, что выходят лишние дома
                                             when p_for_reu is null then 1
                                             when s.for_reu = p_for_reu then 1
                                             else 0 end =1)
          or (var_=1 and t.trest=trest_)
    ...
    null as kr_soi,
    null as fact_cons
    FROM STATISTICS s, S_REU_TREST t, SPUL k, t_org o, t_org_tp ot,
   ...
   (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
    ...
    ELSIF сd_ = '14' THEN
      ...
      from (select e.lsk, case when k.psch in (8) then 2 -- для отображения признаков открытого, старого, закрытого фонда
       ...
       e.usl, e.org, k.nd, k.kw, k.status, k.house_id, u.uslm, g.type,
         ot.name_short||'.'||initcap(o.name)||', '||initcap(s.name)||', '||NVL(LTRIM(k.nd,'0'),'0')||'-'||NVL(LTRIM(k.kw,'0'),'0') as adr,
         ot.name_short||'.'||initcap(o.name)||', '||initcap(s.name) as street1,
          s.name_reu, s.name_tr
          from t_saldo_lsk2 e, kart k, spul s, sprorg g, s_reu_trest s, usl u, t_org o, t_org_tp ot
         where e.lsk = k.lsk and k.reu=s.reu
           and e.org = g.kod
           and k.kul = s.id
           and e.usl = u.usl and s.fk_settlement=o.id and o.fk_orgtp=ot.id
           and exists
          (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
    ...
    insert into temp_stat2
      ...
      select --для отображения объемов по ОДПУ
        ...
        null as lsk,
        t.name_tr as predpr, h.name_reu as reu, 
        ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name)||', '||nvl(ltrim(h.nd,'0'),'0')  as predpr_det,
       ...
       case when show_sal_=0 and i.mg < mg1_ and mg_ <> mg1_ then 0
        ...
        null as fio
        from
        (select distinct e.reu, e.kul, e.nd, e.usl, e.org, e.status, e.fk_lsk_tp, u.type, s.trest, s.name_reu, nvl(u2.parent_usl, e.usl) as parent_usl, u2.uslm
        ...
        (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
         ...
         on i.reu=h2.reu and i.kul=h2.kul and i.nd=h2.nd and i.mg=h2.mg and h.parent_usl=h2.usl*/
          left join temp_stat2 h4 on i.mg=h4.mg and i.reu=h4.reu and i.kul=h4.kul and i.nd=h4.nd 
                  and nvl(h.parent_usl, i.usl)=h4.usl
          left join (select distinct u2.uslm, t.reu, t.kul, t.nd, t.mg, t.ishotpipeinsulated, t.istowelheatexist 
                      from temp_stat2 t, usl u2 where t.usl=u2.usl) h5 on i.mg=h5.mg and i.reu=h5.reu and i.kul=h5.kul and i.nd=h5.nd 
                  and h.uslm=h5.uslm
        ...
        order by i.mg, ot.name_short||'.'||initcap(o.name)||', '||initcap(k.name), utils.f_order(h.nd,6);
      ELSIF det_ in (0, 1) THEN
        -- до ЖЭО
...
--          mg1_, mg_, mg1_, fk_ses_, var_, reu_, var_, trest_, var_, mg_, mg1_;
     ...
     -- Оплата OLAP
      if det_ = 3 then
        ...
        open prep_refcursor for select null as predp,
             ...
             ot.name_short||'.'||initcap(o2.name)||', '||initcap(l.name)||', '||NVL(LTRIM(k.nd,'0'),'0')||'-'||NVL(LTRIM(k.kw,'0'),'0')
             as predpr_det,
             ...
             from kart k, xxito14_lsk s, s_reu_trest t, sprorg r, variant_xxito10 v, spul l, oper o, usl u, t_org o2, t_org_tp ot
             ...
             and l.fk_settlement=o2.id and o2.fk_orgtp=ot.id   
        ...
        -- детализация до домов
         ...
         ot.name_short||'.'||initcap(o2.name)||', '||initcap(k.name)||', '||NVL(LTRIM(s.nd,'0'),'0')
         as predpr_det,
         null as kw, s.var, s.dopl,
         substr(s.dopl, 1, 4)||'-'||substr(s.dopl, 5, 2) as dopl_name,
         s.mg, substr(s.mg, 1, 4)||'-'||substr(s.mg, 5, 2) as mg_name,
         ...
         decode(s.cd_tp, 0, 'Пеня', 'Оплата') as cd_tp
         from xxito14 s, s_reu_trest t, sprorg r, variant_xxito10 v, spul k, oper o, usl u, t_org o2, t_org_tp ot
         ...
         and decode(var_,1,trest_,t.trest)=t.trest
         and k.fk_settlement=o2.id and o2.fk_orgtp=ot.id   
     ...
     if var_ = 3 then
        ...
        --(все тресты)
          ...
          'select sum(summa) as summa, sum(penya) as penya, opername,
               ...
               order by a.dat_ink, a.nkom, opername, a.nink, a.dtek'
     ...
     if var_ = 3 then
        ...
        select d.id, s.name_reu, u.nm, t.name as type_ch,
           ot.name_short||'.'||initcap(o.name)||', '||initcap(l.name)||', '||NVL(LTRIM(k.nd,'0'),'0')||'-'||NVL(LTRIM(k.kw,'0'),'0') as adr,
           c.mgchange, substr(c.mgchange, 1, 4)||'-'||substr(c.mgchange, 5, 2) as mg1,
           c.dtek, u.name, sum(c.summa) as summa, max(c.proc) as proc
            from kart k, spul l, c_change_docs d, c_change c, s_reu_trest s, t_user u, usl u, c_change_tp t, t_org o, t_org_tp ot
            where k.lsk=c.lsk and k.kul=l.id and k.reu=s.reu and d.id=c.doc_id and c.type=t.id and d.user_id=u.id and c.usl=u.usl
            and l.fk_settlement=o.id and o.fk_orgtp=ot.id
           group by d.id,
...
select t.trest||'' ''||t.name_reu as predp,
 ...
 --Отчет по проживающим, для паспортного стола
    ...
    open prep_refcursor for
      ...
      p.fio, p.dat_rog, p.dok_c, p.dok_n, p.dok_v, p.dok_d, p.dok_div, p.dok_inn, p.dok_snils
      from arch_kart s, a_kart_pr2 p, spul l
      where s.mg between mg_ and mg1_ and s.lsk=p.lsk and s.kul=l.id and s.mg between p.mgFrom and p.mgTo
...
--       having sum(summa) > 0; ред 03.10.2011
 ...
 --Отчет по прописанным/выписанным, для паспортного стола
    ...
    open prep_refcursor for
      select
      s.lsk, p.dat_rog, p.dok_death_c, p.dok_death_n, u.name as reason, l.name||', '||NVL(LTRIM(s.nd,'0'),'0')||'-'||NVL(LTRIM(s.kw,'0'),'0') as adr,
      p.fio, r.name as rel, p.dat_prop as dt1, p.dat_ub as dt2
      from arch_kart s, a_kart_pr2 p, spul l, relations r, u_list u
    ...
    open prep_refcursor for
      ...
      s.lsk, p.dat_rog, p.dok_death_c, p.dok_death_n, u.name as reason, l.name||', '||NVL(LTRIM(s.nd,'0'),'0')||'-'||NVL(LTRIM(s.kw,'0'),'0') as adr,
      p.fio, r.name as rel, p.dat_prop as dt1, p.dat_ub as dt2
      from arch_kart s, a_kart_pr2 p, spul l, relations r, s_reu_trest t, u_list u
      where s.mg=mg_ and s.lsk=p.lsk and s.kul=l.id and s.mg between p.mgFrom and p.mgTo and p.relat_id=r.id(+)
...
--      and k.lsk=''00107244''
 ...
 --Новый список-оборотка для субсидирования ТСЖ
    ...
    (select s.lsk, u.uslg,
     sum(case when u.cd in ('отоп')/*u.usl_norm = 0 and s.kpr <> 0*/ then s.summa else 0 end) as summa_n,
     sum(case when u.cd in ('отоп/св.нор')/*u.usl_norm = 1 and s.kpr <> 0*/ then s.summa else 0 end) as summa_sv,
     sum(case when u.cd in ('отоп/0 зарег.')/*s.kpr = 0*/ then s.summa else 0 end) as summa_empt,
     sum(case when u.cd in ('отоп')/*u.usl_norm = 0 and s.kpr <> 0*/ then s.test_opl else 0 end) as opl_n,
     sum(case when u.cd in ('отоп/св.нор')/*u.usl_norm = 1 and s.kpr <> 0*/ then s.test_opl else 0 end) as opl_sv,
     sum(case when u.cd in ('отоп/0 зарег.')/*s.kpr = 0*/ then s.test_opl else 0 end) as opl_empt
     ...
     group by s.lsk, u.uslg) e,
      (select s.lsk, u.uslg, sum(s.summa) as chng from a_change s, usl u where s.mg between mg_ and mg_ and s.usl=u.usl and
       ...
       group by s.lsk, u.uslg) d,
    (select s.lsk,
     sum(case when s.sch <> 0 and u.cd in ('г.вода')/*u.usl_norm = 0 and nvl(s.kpr,0) <> 0*/ then s.summa else 0 end) as summa_sch_n,
     sum(case when s.sch <> 0 and u.cd in ('г.вода/св.нор')/*u.usl_norm = 1 and nvl(s.kpr,0) <> 0*/ then s.summa else 0 end) as summa_sch_sv,
     sum(case when s.sch <> 0 and u.cd in ('г.вода/0 зарег.')/*nvl(s.kpr,0) = 0*/ then s.summa else 0 end) as summa_sch_empt,
     sum(case when s.sch <> 0 and u.cd in ('г.вода')/*u.usl_norm = 0 and nvl(s.kpr,0) <> 0*/ then s.test_opl else 0 end) as vol_sch_n,
     sum(case when s.sch <> 0 and u.cd in ('г.вода/св.нор')/*u.usl_norm = 1 and nvl(s.kpr,0) <> 0*/ then s.test_opl else 0 end) as vol_sch_sv,
     sum(case when s.sch <> 0 and u.cd in ('г.вода/0 зарег.')/*nvl(s.kpr,0) = 0*/ then s.test_opl else 0 end) as vol_sch_empt,
     sum(case when s.sch = 0 and  u.cd in ('г.вода')/*u.usl_norm = 0*/ then s.summa else 0 end) as summa_norm_n,

     sum(case when s.sch = 0 and u.cd in ('г.вода')/*u.usl_norm = 0*/ then s.test_opl else 0 end) as vol_norm_n
       from a_charge2 s, usl u where mg_ between s.mgFrom and s.mgTo and s.usl=u.usl and
 ...
 --Реестр для фонда Капремонта
    ...
    --по Городу
      if utils.get_int_param('CAP_VAR_REP1') = 0 then
          ...
          open prep_refcursor for
            ...
            and (k.house_id in (39986,39966) and k.psch not in (8,9) or -- кроме двух домов выбранных Полыс ред.25.09.20
            k.house_id not in (39986,39966)
            or k.lsk in ('06000671') --просит Полыс выгружать ред. 02.02.21
            and (nvl(t.indebet,0)<>0 or nvl(t.inkredit,0)<>0 or nvl(t.outdebet,0)<>0 or nvl(t.outkredit,0)<>0 or nvl(t.pinsal,0)<>0 or nvl(t.poutsal,0)<>0
                or k.psch not in (8,9) or k.status=1)) -- открытые или с наличием сальдо по кап. или муницип квартиры ред.25.09.20 по просьбе Полыс
    ...
    --по Дому
      ...
      select u.uslm, e.reu, e.kul, e.nd, e.status, e.org, e.usl,  
             trim(t.name_reu) as name_reu, ot.name_short||'.'||initcap(o.name)||', '||initcap(s.name) as name, 
             e.fk_lsk_tp from t_saldo_reu_kul_nd_st e, s_reu_trest t, spul s, usl u, t_org o, t_org_tp ot
      where e.usl=u.usl and e.reu=t.reu and e.reu=reu_ and e.kul=kul_ and e.nd=nd_ and s.fk_settlement=o.id and o.fk_orgtp=ot.id
            and exists
           (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
                and p.id=i.fk_par and p.cd='REP_USL2' 
                and i.sel_cd=e.usl
 ...
 --Оборотная ведомость по домам
    IF var_ = 3 THEN
      --по дому
      OPEN prep_refcursor FOR select h.name_reu as predpr, h.name as adr, sum(i.indebet) as indebet, sum(i.inkredit) as inkredit,
      sum(o.charges) as charges, sum(o.changes) as changes, sum(o.subsid) as subsid, sum(o.privs) as privs, sum(o.privs_city) as privs_city, sum(o.payment) as payment,
      sum(o.ch_full) as ch_full, sum(o.changes2) as changes2, sum(nvl(o.changes,0)+nvl(o.changes2,0)) as changesall,
      sum(o.pn) as pn, sum(u.outdebet) as outdebet, sum(u.outkredit) as outkredit
      from
      (
      select distinct e.reu, e.kul, e.nd, e.org, e.usl, e.status, u2.uslm, trim(t.name_reu) as name_reu, 
       ot.name_short||'.'||initcap(o.name)||', '||initcap(s.name)||', '||ltrim(e.nd,'0') as name, e.fk_lsk_tp 
       from t_saldo_reu_kul_nd_st e, usl u2, s_reu_trest t, spul s, t_org o, t_org_tp ot
      where e.usl=u2.usl and e.reu=t.reu and e.reu=reu_ and e.kul=kul_ and e.nd=nd_ and s.fk_settlement=o.id and o.fk_orgtp=ot.id
            and exists
           (select * from list_c i, spr_params p where i.fk_ses=fk_ses_
                and p.id=i.fk_par and p.cd='REP_USL2' 
                and i.sel_cd=e.usl
    ...
    ELSIF var_ = 2 THEN
      --по ЖЭО
      OPEN prep_refcursor FOR select l.name||h.name_reu as predpr, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0') as adr, sum(i.indebet) as indebet, sum(i.inkredit) as inkredit,
      sum(o.charges) as charges, sum(o.changes) as changes, sum(o.subsid) as subsid, sum(o.privs) as privs, sum(o.privs_city) as privs_city, sum(o.payment) as payment,
      sum(o.ch_full) as ch_full, sum(o.changes2) as changes2, sum(nvl(o.changes,0)+nvl(o.changes2,0)) as changesall,
      sum(o.pn) as pn, sum(u.outdebet) as outdebet, sum(u.outkredit) as outkredit
      from
      ...
      group by reu,kul,nd,org,usl,status, fk_lsk_tp) o,
      (select * from xitog3 e where e.mg=mg1_) u,
      sprorg d, uslm m, org l, spul p, t_org o2, t_org_tp ot
      where h.kul=p.id and p.fk_settlement=o2.id and o2.fk_orgtp=ot.id and
      h.reu=i.reu(+) and h.kul=i.kul(+) and h.nd=i.nd(+) and h.org=i.org(+) and h.usl=i.usl(+) and h.status=i.status(+) and h.fk_lsk_tp=i.fk_lsk_tp(+) and 
      h.reu=o.reu(+) and h.kul=o.kul(+) and h.nd=o.nd(+) and h.org=o.org(+) and h.usl=o.usl(+) and h.status=o.status(+) and h.fk_lsk_tp=o.fk_lsk_tp(+) and
      h.reu=u.reu(+) and h.kul=u.kul(+) and h.nd=u.nd(+) and h.org=u.org(+) and h.usl=u.usl(+) and h.status=u.status(+) and h.fk_lsk_tp=u.fk_lsk_tp(+) and
      l.id=3 and h.org=d.kod and h.uslm=m.uslm
      group by l.name||h.name_reu, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0')
      having sum(i.indebet) <>0 or sum(i.inkredit) <>0 or
      sum(o.charges) <>0 or sum(o.changes) <>0 or sum(o.subsid) <>0 or
      sum(o.privs) <>0 or sum(o.privs_city) <>0 or sum(o.payment) <>0 or
      sum(o.pn) <>0 or sum(u.outdebet) <>0 or sum(u.outkredit) <>0
      order by l.name||h.name_reu, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0');
    ELSIF var_ = 1  THEN
      --по Фонду
      OPEN prep_refcursor FOR select l.name||t.name_tr as predpr, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0') as adr, sum(i.indebet) as indebet, sum(i.inkredit) as inkredit,
      sum(o.charges) as charges, sum(o.changes) as changes, sum(o.subsid) as subsid, sum(o.privs) as privs, sum(o.privs_city) as privs_city, sum(o.payment) as payment,
      sum(o.ch_full) as ch_full, sum(o.changes2) as changes2, sum(nvl(o.changes,0)+nvl(o.changes2,0)) as changesall,
      sum(o.pn) as pn, sum(u.outdebet) as outdebet, sum(u.outkredit) as outkredit
      from
      ...
      group by reu,kul,nd,org,usl,status, fk_lsk_tp) o,
      (select * from xitog3 e where e.mg=mg1_) u,
      sprorg d, uslm m, org l, spul p, s_reu_trest t, t_org o2, t_org_tp ot
      where h.kul=p.id and h.reu=t.reu and p.fk_settlement=o2.id and o2.fk_orgtp=ot.id and 
      h.reu=i.reu(+) and h.kul=i.kul(+) and h.nd=i.nd(+) and h.org=i.org(+) and h.usl=i.usl(+) and h.status=i.status(+) and h.fk_lsk_tp=i.fk_lsk_tp(+) and 
      h.reu=o.reu(+) and h.kul=o.kul(+) and h.nd=o.nd(+) and h.org=o.org(+) and h.usl=o.usl(+) and h.status=o.status(+) and h.fk_lsk_tp=o.fk_lsk_tp(+) and
      h.reu=u.reu(+) and h.kul=u.kul(+) and h.nd=u.nd(+) and h.org=u.org(+) and h.usl=u.usl(+) and h.status=u.status(+) and h.fk_lsk_tp=u.fk_lsk_tp(+) and
      l.id=2 and h.org=d.kod and h.uslm=m.uslm
      group by l.name||t.name_tr, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0')
      having sum(i.indebet) <>0 or sum(i.inkredit) <>0 or
      sum(o.charges) <>0 or sum(o.changes) <>0 or sum(o.subsid) <>0 or
      sum(o.privs) <>0 or sum(o.privs_city) <>0 or sum(o.payment) <>0 or
      sum(o.pn) <>0 or sum(u.outdebet) <>0 or sum(u.outkredit) <>0
      order by l.name||t.name_tr, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0');
    ELSIF var_ = 0 THEN
      --по Городу
      OPEN prep_refcursor FOR select l.name as predpr, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0') as adr, sum(i.indebet) as indebet, sum(i.inkredit) as inkredit,
      ...
      group by l.name, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0')
      having sum(i.indebet) <>0 or sum(i.inkredit) <>0 or
      sum(o.charges) <>0 or sum(o.changes) <>0 or sum(o.subsid) <>0 or
      sum(o.privs) <>0 or sum(o.privs_city) <>0 or sum(o.payment) <>0 or
      sum(o.pn) <>0 or sum(u.outdebet) <>0 or sum(u.outkredit) <>0
      order by l.name, ot.name_short||'.'||initcap(o2.name)||', '||initcap(p.name)||', '||ltrim(h.nd,'0');
...
/*   НЕ удалять пока до 12.06.20!   OPEN prep_refcursor FOR 'select s.reu, s.lsk, '||sql_det||' AS predpr_det,
 ...
 --Задолжники OLAP-3 - Полыс, с развернутыми периодами задолженности
   ...
   select s.lsk, DECODE(k.psch,9,'Закрытые Л/С', 8,'Старый фонд', 'Открытые Л/С') AS psch,
    ...
    case when cur_pay_=1 then s.dolg
      else s.dolg2 end as dolg, s.period_deb, s.deb_month, g.name as deb_org, s.penya, s.nachisl, s.payment, s.dat,
      a.name as st_name
    from kart k, debits_lsk_month s, s_reu_trest t, t_org g, status a
    where decode(l_sel_id,0,2,3)=s.var and decode(l_sel_id,0,k.fk_tp,l_sel_id)=k.fk_tp-- либо совокупно по помещению, либо по конкретному типу лс
    and k.lsk=s.lsk and s.status=a.id and s.reu=t.reu and s.reu=reu_ and s.kul=kul_ and s.nd=nd_ and s.mg between mg_ and mg1_
    and s.fk_deb_org=g.id(+)
    and ((cur_pay_=1 and s.cnt_month > 0) or
    (cur_pay_=0 and s.cnt_month2 > 0))
