...
/*    if lsk_ is not null and lsk1_ is not null then
  ...
  if nvl(var_,0) in (0,4) then --счёт, счёт для УСЗН
   ...
   if mg_ > 200710 and mg_ >= mg_nolg_ then
     --новые счета
     sqlstr4_:='max(psch) in (8,9)  --and k.for_bill=1 - убрал 22.07.21
         and ((sum(y.summa) <> 0 or
         sum(r.summa) <> 0 or
         sum(x.summa) <> 0 or
         sum(sl.summa) <> 0 or
...
sqlstr13_:='select cc2.lsk, cc2.usl, max(cc2.org) as org, sum(cc2.cena) as cena from (
  ...
  sqlstr13_:='select /*+ INDEX (n A_NABOR2_I)*/cc.lsk,
           ...
           where cc.type = 1 and l.mg='''||mg_||''' '||sqlstr_||' and l.psch <> 8
  ...
  OPEN cur FOR
   'select k.mg1, k.mg, k.k_lsk_id, k.lsk, g.name || '' д.'' || ltrim(k.nd, ''0'') || ''-'' || ltrim(k.kw, ''0'') as streetname,
       ...
       max((select count(distinct tt.id)
        from a_kart_pr2 tt, a_lg_docs d, a_lg_pr p
        where tt.id=d.c_kart_pr_id and d.id=p.c_lg_docs_id
        and tt.lsk=k.lsk
        and d.mg=k.mg and d.mg between tt.mgFrom and tt.mgTo and k.mg=p.mg and p.spk_id<>1
               ...
               select a.*, 'ST00012' || '|Name=' || a.full_name ||
                 ...
                 from (select /*+ USE_HASH(k, sl, p1, p2, p3, p4, p5, p6  )*/
                          ...
                          --                    when b.cnt = 1 then o.raschet_schet2
                           ...
                           left join (select l.lsk, sum(l.summa) as changes -- перерасчеты
                                       from scott.a_change l, temp_lsk tmp
                                      where l.mg = p_mg
                                        and l.lsk = tmp.lsk
                                      group by l.lsk) p6
                             on k.lsk = p6.lsk
                          where k.mg = p_mg
                            and (p_var2 = 1 and k.psch in (8, 9) and
                                ...
                                nvl(p6.changes, 0) <> 0 -- добавил 22.07.21
                                ) or
...
procedure deb(p_k_lsk_id in number,
              p_lsk in kart.lsk%type,
              p_flt_mg_from in varchar2,
              p_flt_mg_to in varchar2,
              p_rfcur out ccur
  ) is
  l_lsk kart.lsk%type;
begin
  if utils.get_int_param('SPR_DEB_VAR') = 1 then
    ...
    open p_rfcur for
      ...
      e.summa as sal
       from
       scott.long_table a ,
      (select mg, sum(summa) as summa from c_chargepay2 t, params p where p.period between t.mgFrom and t.mgTo and
      ...
      where a.mg between coalesce(p_flt_mg_from,a.mg) and coalesce(p_flt_mg_to,a.mg)
      and a.mg=b.mg(+) and a.mg=c.mg(+)
      and a.mg=e.mg(+)
    ...
    open p_rfcur for
      select * from (
      select --k.usl_name_short
      p_java.http_req(p_url => '/getKartShortNames',
                               p_url2 => k.lsk||'/'||k.mg,
                               p_server_url => init.g_java_server_url,
                               tp => 'GET')
      ||'-'||k.lsk as lsk, k.psch, s.name||', '||ltrim(k.nd,'0')||', '||ltrim(k.kw,'0') as adr, k.fio, s.name, k.mg,
      ...
      as dolg_name,
        ...
        scott.init.get_fio as fio_kass
          from (select k1.psch, k1.fk_tp, k1.lsk, k1.c_lsk_id, k1.k_lsk_id, k1.kul, k1.nd, k1.kw, first_value(k1.fio)
             over (order by decode(psch,8,0,9,0,1) desc) as fio, a.mg
                --k1.usl_name_short
                from scott.kart k1, scott.long_table a
                where decode(p_k_lsk_id,0,p_lsk,k1.lsk)=k1.lsk
                and a.mg between coalesce(p_flt_mg_from,a.mg) and coalesce(p_flt_mg_to,a.mg)
               ...
               (select c.lsk, c.mg, sum(c.summa) as summa
                  from c_chargepay2 c, kart k2, params p
