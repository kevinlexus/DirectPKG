...
-- вызывается из триггера c_kart_pr_auid
...
-- Установить квартиросъемщика в лицевой счет. Вызывается по кнопке "Установить собственника" из карточки
...
-- вызывается из триггера c_states_pr_aiud, а так же из gen.go_nye_phase2
...
begin
  ...
  select nvl(count(*), 0)
   ...
   where t.lsk = lsk_;
  if l_cnt > 0 then
    return 'Необходимо удалить внешний лиц.счет!';
  end if;
  ...
  select nvl(count(*), 0)
    into l_cnt
    from long_table o, c_chargepay2 t
   where t.lsk = lsk_ and o.mg between t.mgFrom and t.mgTo
     and o.mg < l_mg;
  if l_cnt > 0 then
  ...
  begin
    ...
    delete from c_chargepay2 t
     where t.lsk = lsk_ and l_mg between t.mgFrom and t.mgTo;
...
begin
  ...
  delete from c_kart_pr t where t.lsk=lsk_;
  delete from saldo_usl t where t.lsk=lsk_;
  delete from c_chargepay2 t where t.lsk=lsk_ and l_mg between t.mgFrom and t.mgTo;
  delete from c_kwtp t where t.lsk=lsk_;
...
return l_cnt;
 ...
 -- вызывается только из триггера c_states_sch_aiud
