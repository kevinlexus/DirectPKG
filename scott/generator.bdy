...
*/
  ...
  begin
    ...
    open prep_refcursor for 'SELECT t.rowid, upper(LTRIM(t.nd, ''0'')) AS nd,
         ot.name_short||''. ''||o.name||'', ''||initcap(s.name) as name,
         ot.name_short||''. ''||o.name||'', ''||initcap(s.name)||'', ''||upper(LTRIM(t.nd, ''0'')) as adr,
         kul as kul_id,
         nd as nd_id,
         t.sel
           FROM list_choices_hs t join spul s on t.kul=s.id
