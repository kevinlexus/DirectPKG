  ...
  BEGIN
    ...
    IF nvl(p_lvl, 0) = 0 THEN
      ...
      IF sptarn_ = 0 AND nvl(koeff_, 0) <> 0 THEN
        --коэфф
        INSERT INTO nabor
          (lsk, usl, org, koeff, norm, dt1, dt2)
          SELECT k.lsk, usl_, org_, koeff_, NULL AS norm, p_dt1, p_dt2
      ...
      ELSIF sptarn_ = 1 AND nvl(norm_, 0) <> 0 THEN
        ...
        INSERT INTO nabor
          (lsk, usl, org, koeff, norm, dt1, dt2)
          SELECT k.lsk, usl_, org_, NULL AS koeff, norm_, p_dt1, p_dt2
            FROM kart k, v_lsk_tp tp
           WHERE NOT EXISTS (SELECT *
                    FROM nabor n
                   WHERE n.lsk = k.lsk
      ...
      ELSIF sptarn_ IN (2, 3, 4) AND nvl(koeff_, 0) <> 0 AND nvl(norm_, 0) <> 0 THEN
        ...
        INSERT INTO nabor
          (lsk, usl, org, koeff, norm, dt1, dt2)
          SELECT k.lsk, usl_, org_, koeff_, norm_, p_dt1, p_dt2
            FROM kart k, v_lsk_tp tp
           WHERE NOT EXISTS (SELECT *
                    FROM nabor n
                   WHERE n.lsk = k.lsk
