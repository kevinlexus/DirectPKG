...
insert into statistics_lsk
   ...
   select k.lsk, k.reu, b.reu as for_reu, k.kul, k.nd, k.kw, n.usl, a.kpr,
     case when k.parent_lsk is not null then 0 -- занулить проживающих в связанном лиц.счете
       ...
       mg_ as mg, null as dat, n.limit,
       a.cena, k.fk_tp,
       --decode(lag(k.lsk || u.uslm, 1) over(order by k.lsk, u.uslm, nvl(a.kpr,0) desc, nvl(a.cnt,0) desc), убрал эксперементально 14.04.2021
       --k.lsk || u.uslm, null, k.opl) as opl,
       a.opl,
       case when nvl(a.cnt,0) <> 0 then 'Есть' else 'Нет' end as is_vol,
        ...
        from arch_kart k
             ...
             left join (select k2.k_lsk_id, max(k2.reu) as reu
                         ...
                         ) b on k.k_lsk_id=b.k_lsk_id  -- обслуживающая фонд УК
             left join (select c.lsk, c.usl, c.org, nvl(c.sch,0) as sch,
                        sum(c.kpr) as kpr, sum(c.kprz) as kprz, sum(c.kpro) as kpro,
                        sum(c.test_opl) as cnt,
                        sum(c.opl) as opl,
                        max(c.test_cena) as cena
                        from a_charge2 c
                       where c.type = 1 and mg_ between c.mgFrom and c.mgTo
                       group by c.lsk, c.usl, c.org, c.sch) a on n.lsk=a.lsk and n.usl=a.usl and n.org=a.org
...
insert into statistics_lsk
   ...
   select k.lsk, k.reu, a.reu as for_reu, k.kul, k.nd, k.kw, n.usl, null as kpr,
       ...
       case when nvl(d.vol,0) <> 0 then 'Есть' else 'Нет' end as is_vol, d.vol
        from (select c.lsk, c.usl, c.org, c.mgchange, sum(c.vol) as vol, nvl(c.sch,0) as sch
             ...
             and init.get_dt_end between n.dt1 and n.dt2 -- на последний день месяца (спорно) ред.01.10.21
              ;
      ...
      insert into statistics
        ...
        select reu, for_reu, kul, nd, usl, sum(kpr), s.is_empt, sum(kpr_ot), sum(kpr_wr),
         ...
         where s.mg = ''||mg_||'' and s.usl is not null
 ...
 for c in (select t.reu,t.kul,t.nd, u.uslm,t.psch,t.rowid as rd from statistics t join usl u
    on t.usl=u.usl
    where t.mg=mg_ and t.usl is not null
  ...
  commit;
  logger.log_(time_, 'gen_stat_usl ' || mg_);
end gen_stat_usl;
