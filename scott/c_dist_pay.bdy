...
function dist(p_mg in varchar2, p_dopl in varchar2, p_summa in number,
...
begin
  ...
  if p_kwtp_mg_id is null then --если нет идентификатора платежа (либо сумма оплаты, либо пеня, с 0 суммой оплаты)
    ...
    --по текущему начислению + перерасчет
      select nvl(count(*),0) into l_cnt
      from (
      ...
      select t.usl, t.org, t.summa
          ...
          where t.lsk=p_rec.lsk) d
        group by d.usl, d.org
        having sum(d.summa) > 0); --только по начислению > 0
  ...
  --по текущему начислению (по тек начисл, без коррект проводки) + перерасчет
    ...
    select rec_summ(d.usl, d.org, sum(d.summa), 0) bulk collect
          into t_summ from
            (select t.usl, t.org, nvl(t.indebet,0) as summa
              from xitog3_lsk t, kart k --взять входящее деб сальдо
              where t.lsk=p_rec.lsk and t.lsk=k.lsk
              and t.mg=p_mg
              and not exists -- неработающие в текущем периоде
              (select * from nabor n where n.lsk=t.lsk and n.usl=t.usl and n.org=t.org)
              --and exists     -- работающие в периоде оплаты ред.30.03.2022 - Екатерина Ивановна сказала отключить условие, посмотреть как будет распр. (Иначе не закрывалось сальдо по 7 орг)
              --(select * from a_nabor2 n where n.lsk=t.lsk and n.usl=t.usl and n.org=t.org and p_dopl between n.mgfrom and mgto)
              ) d
             group by d.usl, d.org
             having  sum(summa ) > 0;
    if sql%rowcount =0 then
...
--7 - распределено по текущему начислению + перерасчет
...
--16 - распределено по текущему начислению, без коррект проводки + перерасчет
...
if p_rec.dopl <= l_mg and l_summa > 0 then
  ...
  l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, 18, null);
...
l_summa_tmp:=0;
    loop
      ...
      if l_summa > 0 then
        l_summa_tmp:=dist(l_mg, p_rec.dopl, l_summa, 1, l_dist, null);
      else
        exit;
      end if;
      l_summa:=l_summa-l_summa_tmp;
...
loop
  if l_flag = 0 then
    if l_summa > 0 then
      ...
      if p_rec.dopl>=l_mg then
        ...
        l_summa_tmp:=dist(l_mg, null, l_summa, 1, l_dist, null);
      else
        --архивный период
        if l_flag=1 then
          l_dist:=14; --по сальдо
        ...
        l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, l_dist, null);
      end if;
      l_summa:=l_summa-l_summa_tmp;
    elsif l_summa < 0 then
      --сделал по просьбе Е.И распределение снятия пени
      l_dist:=7;  --по текущему начислению
      l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, l_dist, null); --поправил -1 на 1 ред. 19.11.15
      l_summa:=l_summa-l_summa_tmp;
    end if;
  end if;
  ...
  if l_flagp = 0 then
    if l_summa_p > 0 and l_flag = 0 then
      ...
      l_dist:=6;
      l_summa_tmp:=dist(p_rec.dopl, null, l_summa_p, 0, l_dist, p_rec.id);
      l_summa_p:=l_summa_p-l_summa_tmp;
    elsif l_summa_p < 0 then
      --сделал по просьбе Е.И распределение снятия пени
      l_dist:=6;
      l_summa_tmp:=dist(p_rec.dopl, null, l_summa_p, 0, l_dist, null);
      l_summa_p:=l_summa_p-l_summa_tmp;
    end if;
  end if;
  i:=i+1;
...
if utils.get_int_param('PAY_ORD1') = 1 then
  if l_summa <> 0 then
    ...
    if l_cnt<>0 then
      i:=0;
      loop
        l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, 16, null);
        l_summa:=l_summa-l_summa_tmp;
        i:=i+1;
  ...
  if l_summa <> 0 then
    ...
    i:=0;
    loop
      l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, 15, null);
      l_summa:=l_summa-l_summa_tmp;
      i:=i+1;
      exit when l_summa =0 or i >=500 ;
    end loop;
...
if l_summa <> 0 then
  ...
  i:=0;
  loop
    l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, 17, null);
    l_summa:=l_summa-l_summa_tmp;
    i:=i+1;
    exit when l_summa =0 or i >=500 ;
  end loop;
...
if l_summa <> 0 then
  ...
  i:=0;
  loop
    l_summa_tmp:=dist(p_rec.dopl, null, l_summa, 1, 20, null);
    l_summa:=l_summa-l_summa_tmp;
    i:=i+1;
    exit when l_summa =0 or i >=500 ;
  end loop;
...
if l_summa_p <> 0 then
  ...
  i:=0;
  loop
    l_summa_tmp:=dist(p_rec.dopl, null, l_summa_p, 0, 15, null);
    l_summa_p:=l_summa_p-l_summa_tmp;
    i:=i+1;
...
if l_summa_p > 0 then --если и по nabor не распр, кинуть на УК, тек содерж
  ...
  select
    ...
    --сделал по просьбе Е.И распределение снятия пени
  l_summa_tmp:=dist(p_rec.dopl, null, l_summa_p, 0, 16, null);
end if;
