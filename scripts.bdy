...
procedure swap_sal_from_main_to_rso2 is
 ...
 l_rso number;
begin
  l_mg:='202101'; --тек.период
  l_cd:='swap_sal_from_main_to_RSO2_20210127_1';
  l_mgchange:=l_mg;
  l_dt:=to_date('20210127','YYYYMMDD');
  l_mg3:=utils.add_months_pr(l_mg,1); --месяц вперед

  select t.id into l_main from v_lsk_tp t where t.cd='LSK_TP_RSO';
...
for c in (select k.lsk as lskFrom, k2.lsk as lskTo, a.usl, a.org, 
        a.summa
         ...
         join 
         (select s.lsk, s.usl, s.org, sum(s.summa) as summa from (
           select t.lsk, t.usl, t.org, t.summa from saldo_usl_script t where t.mg='202102'
           and t.usl in ('007','056','015','058','094') and t.org=4
               ) s
          group by s.lsk, s.usl, s.org) a on k.lsk=a.lsk and a.summa < 0
        where k.fk_tp=l_main and k2.fk_tp=l_rso
        --k.psch not in (8,9)
        --and 
         and k.reu='014' -- источник
